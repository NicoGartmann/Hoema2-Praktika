...
CMyVektor CMyVektor::gradient(CMyVektor x, double(*funktion)(CMyVektor x)) {
	...
	for (int i = 0; i < x.getDimension(); i++) {
		...
		//berechne numerische Ableitung durch Formel: f(x1,...,xi-1+h,xi+1,...,xn)-f(x1,...,xn)/h
...
void CMyVektor::maximieren(double(*funktion)(CMyVektor x), CMyVektor x, double lambda) {
	...
	//Abbruchbedingung: solange bis ||gradf(x)||<10*e-5 oder 50 Schritte gemacht wurden 
	while (schritte < 50 && gradient(x, (*funktion)).getLength() > 0.00001) {
		...
		if ((*funktion) (xneu) > (*funktion) (x)) {
			...
			//test mit doppelter Schrittweite
			...
			//Falls f(xTest)>f(xNeu) - doppelte Schrittweite 
		...
		//falls f(xNeu)<=f(x)
			...
			//lambda solange Halbieren, bis f(xNeu)>f(x)
			//überneheme neues lambda und xNeu als x
